...
fn main() -> Result<()> {
    ...
    for (mod_name, stored_skel) in active_skeletons.iter() {
        ...
        if let Ok(alerts_map) = bpf_loader::get_map(&**stored_skel, "alerts") {
            ...
            println!("· [Warning] Standard 'alerts' map not found in module: {}", mod_name);
        }
    ...
    let ring_buffer = if has_ringbuf {
        Some(ringbuf_builder.build().context("Failed to build unified BPF RingBuffer")?)
    } else {
                    ...
                    if let Some(skel) = active_skeletons.get(&target) {
                        ...
                        match bpf_loader::set_module_state(&**skel, &target, true) {
                            Ok(_) => {
                                ...
                                format!("SUCCESS: Defense module '{}' ENABLED via state synchronization\n", target)
                            }
                        ...
                        match bpf_loader::load_module(&target) {
                            Ok(skel) => {
                                if let Err(e) = bpf_loader::set_module_state(&*skel, &target, true) {
                                    format!("ERROR: Failed to set kernel state for {}. Aborting: {}\n", target, e)
                                } else {
                                    active_skeletons.insert(target.clone(), skel);
                                    if let Some(user_mod) = shared_registry.iter().find(|m| m.slug() == target) {
                                        user_mod.toggle(true);
                                    }
                                    format!("SUCCESS: Defense module '{}' ENABLED and ATTACHED\n", target)
                                }
                            }
                    ...
                    if let Some(skel) = active_skeletons.get(&target) {
                        match bpf_loader::set_module_state(&**skel, &target, false) {
                            Ok(_) => {
                                ...
                                format!("SUCCESS: Defense module '{}' DISABLED via state synchronization\n", target)
                            }
                            Err(e) => format!("ERROR: Failed to update kernel state for '{}': {}\n", target, e),
                        }
                    } else {
