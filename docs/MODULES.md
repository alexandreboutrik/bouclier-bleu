...
`Bouclier Bleu` currently operates across **33 eBPF hooks**, driving **10 active security detectors** (modules) that map directly to **19 MITRE ATT&CK techniques**. Its stability and regression prevention are guaranteed by a suite of **107 automated tests**, encompassing 19 unit, 80 component, 4 integration, and 4 benchmark validation pipelines.
