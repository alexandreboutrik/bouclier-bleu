...
use crate::{
	dump_restrict, exec_block, io_restrict, madvise_ratelimit, mount_secure, ptrace_block,
	rename_entropy, shield, strict_wx, userns_restrict,
};
use std::sync::Arc;
...
pub fn build_registry() -> Vec<Arc<dyn SecurityModule + Send + Sync>> {
	vec![
		...
		Arc::new(madvise_ratelimit::MadviseRatelimit::new()),
		Arc::new(io_restrict::IoRestrict::new()),
