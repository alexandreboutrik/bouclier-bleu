...
/// Discards timings and spools the CPU up to Turbo and warms caches.
/// We use `cat` to natively generate a pipe (`S_ISFIFO`) without relying on
/// FFI `libc`.
fn execute_warmup(count: usize) {
	let mut child = Command::new("cat")
		.stdin(Stdio::piped())
		.stdout(Stdio::null())
		.spawn()
		.expect("Failed to spawn warmup child");
...
/// Dynamically extracts module slugs from the CLI list output using structural
/// parsing.
...
#[test]
fn benchmark_io_restrict_overhead() {
	/*
	 * WORKER SUBPROCESS TRAP
	 * Catches re-invocations of this binary for Phase 3. This allows us to run
	 * the pipe write loop as an unprivileged user (nobody) while measuring the
	 * exact duration *internally* to completely bypass process spawn overhead.
	 */
	if let Ok(_batch_str) = std::env::var("PHASE3_BATCH") {
		let mut child = Command::new("cat")
			.stdin(Stdio::piped())
			.stdout(Stdio::null())
			.spawn()
			.expect("Failed to spawn phase 3 child");
		...
		println!("bb_nanos:{}", elapsed.as_nanos());
		std::process::exit(0);
	}
	...
	execute_warmup(WARMUP_ITERATIONS);

	let mut baseline_durations = Vec::new();
	let batches = ITERATIONS / BATCH_SIZE;

	for _ in 0..batches {
		let mut p_child = Command::new("cat")
			.stdin(Stdio::piped())
			.stdout(Stdio::null())
			.spawn()
			.expect("Failed to spawn baseline child");
		let mut stdin = p_child.stdin.take().unwrap();
		let buf = [0u8; 1];
	...
	/*
	 * PHASE 2 & 3: INTERLEAVED EXECUTION (io_restrict ENABLED)
	 */
	println!("[INFO] Enabling io_restrict module via IPC...");
	let _ = Command::new(&cli_bin)
		.args(["enable", "io_restrict"])
		.output();
	thread::sleep(Duration::from_millis(500));

	println!("[INFO] Executing Phase 2 & 3: Fast-Path vs LRU Map Lookup Measurement");

	println!(
		"  -> Running {} post-activation warmup operations...",
		WARMUP_ITERATIONS
	);
	execute_warmup(WARMUP_ITERATIONS);
	...
	for b in 0..batches {
		// Phase 2: Fast-Path Deferral (Root Bypass)
		let mut p_child = Command::new("cat")
			.stdin(Stdio::piped())
			.stdout(Stdio::null())
			.spawn()
			.expect("Failed to spawn fast-path child");
		let mut stdin = p_child.stdin.take().unwrap();
		let buf = [0u8; 1];
		...
		// Phase 3: Taint Verification / LRU Map Lookup (Unprivileged User)
		// Re-invoke the test binary as UID 65534 (nobody).
		let output = Command::new(&current_exe)
			.arg("benchmark_io_restrict_overhead")
			.arg("--exact")
			.arg("--nocapture")
		...
		if let Some(nanos) = parsed_nanos {
			...
			panic!(
				"Phase 3 worker failed. Stdout: {}, Stderr: {}",
				stdout,
	...
	let calc_ops = |dur: Duration| -> f64 { ITERATIONS as f64 / dur.as_secs_f64() };

	let calc_ns_op = |dur: Duration| -> f64 { dur.as_nanos() as f64 / ITERATIONS as f64 };
	 ...
	 */
	let report = format!(
        "\n### Syscall Overhead: `io_restrict` module (Median)\n\n\
        | Phase | Cleaned Time | Throughput | Latency / Op | Overhead |\n\
        |-------|--------------|------------|--------------|----------|\n\
        | **1. Baseline (Modules Disabled)** | `{:.2}ms` | `{:.0} ops/sec` | `{:.0} ns` | `-` |\n\
        | **2. Fast-Path (Root Bypass)** | `{:.2}ms` | `{:.0} ops/sec` | `{:.0} ns` | `+{:.0} ns` |\n\
        | **3. Taint Verification (LRU Map Lookup)** | `{:.2}ms` | `{:.0} ops/sec` | `{:.0} ns` | `+{:.0} ns` |\n\n",
	...
	file.write_all(report.as_bytes())
		.expect("Failed to append io_restrict benchmark results");
}
