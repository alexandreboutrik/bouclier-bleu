...
fn benchmark_rename_entropy_overhead() {
	...
	for b in 0..batches {
		...
		for i in 0..BATCH_SIZE {
			...
			let new_path = format!(
				"{}/low_entropy_dummy_data_{}_renamed.txt",
				UNMONITORED_DIR, idx
			);
			let _ = fs::rename(&old_path, &new_path);
		}
	...
	let unmonitored_duration =
		unmonitored_durations[unmonitored_durations.len() / 2] * (batches as u32);
