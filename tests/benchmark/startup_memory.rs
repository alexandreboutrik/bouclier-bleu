...
fn benchmark_startup_and_memory() {
    ...
    if startup_duration == Duration::ZERO {
        ...
        let _ = child.kill();
        panic!("Daemon failed to bind socket within timeout. Did it crash? Status: {:?}", status);
    }
    ...
    let prog_output = Command::new("bpftool").args(["prog", "show", "-j"]).output().expect("bpftool prog failed");
    let map_output = Command::new("bpftool").args(["map", "show", "-j"]).output().expect("bpftool map failed");
    ...
    if active_modules.is_empty() {
        ...
        for slug in active_modules {
            ...
            report.push_str(&format!("* **`{}`:** `{:.2} KB`\n", slug, mem_bytes as f64 / 1024.0));
        }
    }
